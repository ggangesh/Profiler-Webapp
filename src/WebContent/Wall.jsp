...
$("#postComment_"+post_id).submit(function(e){
	$.ajax({
		...
		{
			$("#viewCommentBtn_"+post.post_id).click();
		},
		error: function(){
...
function postComment(post_id) {
	$.post("PostComment",$("#postComment_"+post_id).serialize(), function(response){
	});
}
</script>
<script>
  $.ajax({
      ...
      success : function(data) {
		...
		error: function(){
			alert("Error in ajax call for getting timeline");
		}
  });
  </script>
...
<div id="name">
<a href="MyWall">
<img src="http://${serverIp}/DBProjectImages/${profile_pic}" alt="Wall photo of ${myname}" height="30" align="middle" style="border-radius: 10px;"> ${myname}</a>
</div>
<div id="logout">
...
<div id="leftside">
	<div id="dp">
	<div id="dpin">
	<img src="http://${serverIp}/DBProjectImages/${group_photo}" alt="Wall photo of ${name}" width="182" style="border-radius: 25px;">
	</div>
	</div>
	<p style="align:center;"><h1>${name}</h1></p>
	<div id="follow">
	<%
	if((boolean)request.getAttribute("follow"))
	{%>
		<form method="post" action="ToggleFollow" >
		<input type="hidden" name="wallIdHidden" value="<%=request.getSession().getAttribute("last_wall_id")%>">
		<button type="submit" name="follow_btn" value="unfollow_btn" style="width: 110px;height: 30px; background-image: url(http://${serverIp}/DBProjectImages/unfollow.png); background-repeat: no-repeat;">
		</button>
		</form>
	<%}
	else{%>
		<form method="post" action="ToggleFollow" >
		<input type="hidden" name="wallIdHidden" value="<%=request.getSession().getAttribute("last_wall_id")%>">
		<button type="submit" name="follow_btn" value="follow_btn" style="width: 80px;height: 30px; background-image: url(http://${serverIp}/DBProjectImages/follow.png); background-repeat: no-repeat;">
		</button>
		</form>	<%}%>
	${follow_output}
	</div>
	<div id="description">
		...
		<div id="descriptionContent"><p>${group_description}</p></div>
	</div>
</div>
...
<div id="postformout">
	<div id="postform">
		...
		<input type="hidden" name="postWallId" value="<%=request.getSession().getAttribute("last_wall_id")%>" />  
